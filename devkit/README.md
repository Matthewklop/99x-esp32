...
Plug in your ESP32-C3, run `make flash`. You'll see `99x alive` over USB serial.
...
MIT
=======
# 99x-esp32 — DevKit Edition

**ESP32 DevKit rev1 bare-metal firmware. 320 bytes. No SDK. No Arduino. No ESP-IDF.**

Drives the onboard LED (GPIO2). UART output is WIP due to dual-core contention.
...
Any ESP32 DevKit with CP2102 USB-UART:
- ESP32-DevKitC
- ESP-WROOM-32
...
- GPIO output ✓
- WDT feed ✓
- CPU core detection (PRID) ✓

## What's in progress

- UART output (APP CPU writes to same FIFO)
- UART baud config (clock source differs per board)
